...
class RevioConnector(Connector):
    ...
    def __init__(self) -> None:
        ...
        # Customer resolution is used by search, contacts, services, ledger,
        # invoices, and Customer 360. Cache the Billing customer directory for
        # five minutes so every partial-name lookup does not repaginate Rev.io.
        self._billing_customer_directory_cache: list[dict[str, Any]] = []
        self._billing_customer_directory_cached_at: float = 0.0
        self._billing_customer_directory_lock = asyncio.Lock()
        self._billing_customer_directory_ttl_seconds = 300.0
    ...
    async def resolve_customer(
        ...
        resolution = await self.resolve_customer_query(customer_name)

        if resolution.get("resolved"):
            psa_customer_id = resolution.get("psa_customer_id")
            psa_customer = resolution.get("psa_customer")

            if psa_customer_id is not None:
                return {
                    "resolved": True,
                    "customer_id": int(psa_customer_id),
                    "customer_name": resolution.get("customer_name"),
                    "customer": psa_customer or {},
                    "billing_customer_id": resolution.get("billing_customer_id"),
                    "entity_resolution": resolution,
                }

        matches = [
            match.get("psa_customer")
            or match.get("billing_customer")
            or {
                "customer_name": match.get("customer_name"),
                "customer_id": match.get("psa_customer_id"),
                "billing_customer_id": match.get("billing_customer_id"),
            }
        ...
        return {
            ...
            "reason": (
                ...
                or "The customer could not be resolved to Rev.io PSA."
            ),
            "matches": matches,
            "entity_matches": resolution.get("matches") or [],
            "selection_required": bool(
                resolution.get("selection_required")
            ),
            "entity_resolution": resolution,
        }
    ...
    async def search_billing_customers(
        ...
        *,
        ...
        if query:
            # Rev.io's name search is used for candidate discovery only.
            # Candidates are hydrated and locally verified below.
        ...
        rows = self._billing_rows(payload)

        if not query or not rows:
            return rows

        async def hydrate(row: dict[str, Any]) -> dict[str, Any]:
            cid = self._billing_customer_id(row)
            if cid is None:
                return row

            try:
                detail = await self.get_billing_customer(cid)
            except Exception:
                return row
        ...
        def customer_search_text(row: dict[str, Any]) -> str:
            values: list[str] = []

            # Only fields that represent the customer/company name should
            # determine a name-search match. Do not search the entire JSON
            # payload because hidden metadata can create false positives.
            for key in (
                "Name",
                "name",
                "CustomerName",
        ...
        return [
            row
            for row in hydrated
            if needle in customer_search_text(row)
        ]
    ...
    def _billing_customer_name(customer: dict[str, Any]) -> str:
        # Some Rev.io Billing customer payloads do not expose a top-level name.
        # Prefer the common top-level aliases, then fall back to the company
        # name stored on the Billing/Service/Listing address objects.
        for key in (
            "Name",
            "CustomerName",
            "customerName",
        ...
        for address_key in (
            ...
            address = customer.get(address_key)
            if not isinstance(address, dict):
                continue
    ...
    async def get_billing_customer_directory(
        self,
        *,
        refresh: bool = False,
        page_size: int = 500,
        ...
        """Return the complete Rev.io Billing customer directory.

        The directory is cached briefly because customer entity resolution is
        shared by many Ribbit operations. Matching is performed locally
        against actual customer/company names rather than Rev.io search.name.
        """
        now = time.time()
        if (
            not refresh
            and self._billing_customer_directory_cache
            and (
                now - self._billing_customer_directory_cached_at
                < self._billing_customer_directory_ttl_seconds
            )
        ):
            return [dict(row) for row in self._billing_customer_directory_cache]

        async with self._billing_customer_directory_lock:
            now = time.time()
            if (
                not refresh
                and self._billing_customer_directory_cache
                ...
                return [
                    dict(row)
                    for row in self._billing_customer_directory_cache
                ]

            safe_page_size = min(max(page_size, 1), 500)
            safe_max_pages = min(max(max_pages, 1), 100)
            rows: list[dict[str, Any]] = []

            for page in range(1, safe_max_pages + 1):
                page_rows = await self.search_billing_customers(
                    query=None,
                    page=page,
            ...
            self._billing_customer_directory_cache = [
                dict(row)
                for row in rows
            ]
            self._billing_customer_directory_cached_at = time.time()

            return [dict(row) for row in rows]

    async def resolve_customer_query(
        self,
        customer_query: str,
    ) -> dict[str, Any]:
        """Resolve any partial customer inquiry consistently.

        Billing is the primary customer directory. PSA customer records are
        merged when available so downstream operations receive the correct ID
        for each system. Exact-name matches win automatically. One partial
        match resolves automatically. Multiple partial matches are returned
        for user selection.
        """
        query = " ".join(str(customer_query or "").strip().split())
        needle = query.casefold()
        ...
        if not query:
            return {
                ...
                "reason": "A customer name is required.",
                "query": query,
                "matches": [],
            }

        async def billing_directory() -> list[dict[str, Any]]:
            try:
                return await self.get_billing_customer_directory()
            except Exception:
                return []

        async def psa_matches() -> list[dict[str, Any]]:
            try:
                return await self.search_customers(
                    query=query,
                    page=1,
                    per_page=500,
                )
        ...
        billing_rows, psa_rows = await asyncio.gather(
            billing_directory(),
            psa_matches(),
        )

        # Canonical candidate keyed by actual company/customer name.
        ...
        for row in billing_rows:
            ...
            name = self._billing_customer_name(row).strip()
            if (
                not name
                or name.casefold() == "unnamed customer"
                or needle not in name.casefold()
            ):
                continue

            key = name.casefold()
            current = candidates_by_name.setdefault(
                key,
                {
                    "customer_name": name,
                    "billing_customer_id": None,
                    "psa_customer_id": None,
            ...
            is_parent = (
                row.get("is_parent") is True
                or str(row.get("is_parent")).strip().casefold() == "true"
            )
            existing = current.get("billing_customer")
            existing_is_parent = (
                isinstance(existing, dict)
                and (
                    existing.get("is_parent") is True
                    or str(existing.get("is_parent")).strip().casefold()
                    == "true"
                )
            )

            # For duplicate Billing names, prefer the parent account.
            if existing is None or (is_parent and not existing_is_parent):
                current["billing_customer"] = row
                current["billing_customer_id"] = self._billing_customer_id(row)

            if "Rev.io Billing" not in current["sources"]:
                current["sources"].append("Rev.io Billing")

        for row in psa_rows:
            if not isinstance(row, dict):
                continue

            name = self._customer_name(row).strip()
            ...
            current["psa_customer"] = row
            current["psa_customer_id"] = self._customer_id(row)
            if "Rev.io PSA" not in current["sources"]:
                current["sources"].append("Rev.io PSA")

        candidates = sorted(
            candidates_by_name.values(),
            key=lambda item: str(item.get("customer_name") or "").casefold(),
        )

        exact = [
            candidate
            for candidate in candidates
            if str(candidate.get("customer_name") or "").strip().casefold()
            == needle
        ]
        ...
        if chosen is not None:
            return {
                ...
                "billing_customer_id": chosen.get("billing_customer_id"),
                "psa_customer_id": chosen.get("psa_customer_id"),
                "billing_customer": chosen.get("billing_customer"),
                "psa_customer": chosen.get("psa_customer"),
        ...
        return {
            ...
            "reason": (
                "No Rev.io customer name matched the supplied inquiry."
                if not candidates
                else "Multiple Rev.io customers matched the supplied inquiry."
            ),
            "query": query,
    ...
    async def resolve_billing_customer(
        ...
        if resolution.get("resolved"):
            billing_customer_id = resolution.get("billing_customer_id")
            billing_customer = resolution.get("billing_customer")

            if billing_customer_id is not None:
                return {
                    "resolved": True,
                    "customer_id": int(billing_customer_id),
                    "customer_name": resolution.get("customer_name"),
                    "customer": billing_customer or {},
                    "psa_customer_id": resolution.get("psa_customer_id"),
                    "entity_resolution": resolution,
                }

        matches = [
            match.get("billing_customer") or {
                "customer_name": match.get("customer_name"),
                "customer_id": match.get("billing_customer_id"),
                "psa_customer_id": match.get("psa_customer_id"),
            }
            for match in resolution.get("matches") or []
        ]
        ...
        return {
            ...
            "reason": (
                resolution.get("reason")
                or "The customer could not be resolved to Rev.io Billing."
            ),
            "matches": matches,
            "entity_matches": resolution.get("matches") or [],
            "selection_required": bool(
    ...
    async def get_company_billing_ledger(
        ...
        *,
        ...
        """
        Pull account-wide Rev.io Billing charges and credits for a date range.

        This avoids one API request per customer. Records are paged until the
        API returns fewer than page_size rows or max_pages is reached.
        """
        ...
        charges, credits = await asyncio.gather(
            fetch_all(self.search_billing_charges),
            fetch_all(self.search_billing_credits),
        )
        ...
        active_customer_ids = {
            ...
            for entry in entries
            if entry.get("customer_id") not in (None, "")
        }
        ...
        return {
            ...
            "customers_with_activity": len(active_customer_ids),
            "created_date_start": created_date_start,
            "created_date_end": created_date_end,
            "payment_data_included": False,
            "ledger_source": "rest_charges_credits_account_wide",
            "page_size": normalized_page_size,
    ...
    async def get_billing_customer_ledger(
        ...
        *,
        ...
        """
        Return a customer's most recent Rev.io Billing ledger activity.

        REST Charges/Credits are fully paginated instead of stopping at the
        first 500 historical rows. Entries are returned newest-first after the
        running balance is calculated chronologically.

        SOAP Transactions_Query remains preferred when it returns data because
        it can include transaction types such as payments/adjustments. If SOAP
        is unavailable, errors, or returns an empty result, REST is used.
        """
        normalized_page_size = min(max(page_size, 1), 500)
        normalized_max_pages = min(max(max_pages, 1), 100)

        source = "rest_charges_credits_paginated"
        ...
        if not entries:
            ...
            source = (
                "rest_charges_credits_after_empty_soap"
                if settings.revio_transactions_use_soap
                else "rest_charges_credits_paginated"
            )
        else:
            charges = [
                row
                for row in entries
                if str(row.get("entry_type") or "").upper() == "CHARGE"
            ]
            credits = [
                row
                for row in entries
                if str(row.get("entry_type") or "").upper() == "CREDIT"
            ]
        ...
        total_charges = sum(
            float(entry.get("amount") or 0)
            for entry in entries
            if str(entry.get("entry_type") or "").upper() == "CHARGE"
        )
        total_credits = sum(
            float(entry.get("amount") or 0)
            for entry in entries
            if str(entry.get("entry_type") or "").upper() == "CREDIT"
        )
        total_payments = sum(
            float(entry.get("amount") or 0)
            for entry in entries
            if str(entry.get("entry_type") or "").upper() == "PAYMENT"
        )

        oldest_created_date = (
            entries[0].get("created_date")
            if entries
            else None
        )
        newest_created_date = (
            entries[-1].get("created_date")
            if entries
            else None
        )

        # Customer 360 should see current activity first.
        entries.reverse()
        ...
        return {
            ...
            "latest_running_balance": round(running_balance, 2),
            "oldest_created_date": oldest_created_date,
            "newest_created_date": newest_created_date,
            "payment_data_included": total_payments > 0,
