...
class RevioConnector(Connector):
    ...
    async def search_customers(
        ...
        *,
        ...
        # Rev.io's customer-list route accepts GET without the ticket-style
        # page/perPage query parameters. Some tenants return 405 when those
        # parameters are included, so retrieve the list first and filter locally.
        payload = await self._request(
            "GET",
            "/billing/api/v1/customers",
        )
        ...
        if query:
            customers = [
                customer
                for customer in customers
                if self._contains(customer, query)
            ]
    ...
    async def resolve_customer(
        ...
        if len(candidates) == 1:
            ...
            if customer_id is None:
                return {
                    "resolved": False,
                    "reason": "The matching customer did not include a numeric customer ID.",
                    "matches": candidates,
                }
