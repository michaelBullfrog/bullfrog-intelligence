...
class RevioConnector(Connector):
    ...
    async def resolve_billing_customer(
        ...
        # Rev.io parent accounts often return alongside all of their
        # child/location accounts. Prefer one exact-name parent account.
        exact_parent_matches = [
            customer
            for customer in exact_matches
            if customer.get("is_parent") is True
            or str(customer.get("is_parent")).strip().casefold() == "true"
        ]

        if len(exact_parent_matches) == 1:
            customer = exact_parent_matches[0]
        elif len(exact_matches) == 1:
            customer = exact_matches[0]
        else:
            # If there is not one exact-name match, accept a single obvious
            # parent among the partial-name results. Otherwise keep asking for
            # clarification so unrelated customers are not selected.
            parent_matches = [
                customer
                for customer in customers
                if customer.get("is_parent") is True
                ...
                return {
                    ...
                    "reason": (
                        ...
                        if not customers
                            ...
                            "Multiple Rev.io Billing customers matched the "
                            "supplied name."
                        )
                    ),
                    "matches": customers[:25],
                }
