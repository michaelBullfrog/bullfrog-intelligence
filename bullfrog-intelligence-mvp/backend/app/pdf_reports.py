...
def _records(data: dict[str, Any]) -> tuple[str, list[dict[str, Any]]]:
    candidates = [
        ...
        ("Billing Ledger", data.get("ledger_entries")),
...
def _columns(title: str) -> list[tuple[str, list[str]]]:
    mapping: dict[str, list[tuple[str, list[str]]]] = {
        ...
        "Billing Ledger": [
            ("Date", ["created_date", "CreatedDate", "createdDate"]),
            ("Type", ["entry_type", "Type", "type"]),
            ("Description", ["description", "Description"]),
            ("Bill", ["bill_id", "BillID", "billId"]),
...
def create_pdf_report(
    *,
    ...
    elif title == "Billing Ledger" and data.get("customer_name"):
        display_title = f"{data['customer_name']} Charge and Credit Ledger"
    ...
    if title == "Billing Ledger":
        story.extend(
            [
                Paragraph(
                    "Note: This report currently contains charges and credits. "
                    "Payment receipt and payment-application records are not "
    ...
    if rows:
        ...
        for row in rows:
            ...
            for label, keys in columns:
                if title == "Billing Ledger":
                    raw_value = _pick(row, keys)
                    if label in ("Amount", "Running Total"):
                        value = _currency(raw_value)
                    else:
                        value = raw_value
                elif title == "Service Products":
                    kind = None
                    if label == "Rate":
                        kind = "currency"
                    elif label == "Quantity":
