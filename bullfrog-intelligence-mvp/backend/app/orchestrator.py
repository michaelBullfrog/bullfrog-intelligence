...
def _fallback_summary(
    ...
    if operation == "revio_billing_search_service_products":
        ...
        breakdown = data.get("quantity_breakdown") or []
        breakdown_lines = []
        for row in breakdown[:12]:
            description = str(row.get("description") or "Unnamed service")
            ...
            breakdown_lines.append(
                f"• {description}: {quantity_text}"
            )

        total_text = (
            str(int(total_quantity))
            if total_quantity.is_integer()
            else f"{total_quantity:,.2f}"
        )

        scope = "active/current " if active_only_requested else ""
        answer = (
            f"Rev.io shows {total_text} {scope}service-product units for "
            f"{customer_name} across {len(products):,} line items."
        )
        ...
        answer += (
            "\n\nThis answer was calculated directly from Rev.io Billing "
            "without requiring the OpenAI planner."
        )
        return answer
...
def _fallback_plan_from_message(message: str) -> dict[str, Any]:
    ...
    if any(term in lowered for term in license_terms):
        ...
        patterns = (
            ...
            r"(?:licenses?|seats?|current services|active services|service products)\s+(?:for|at|of)\s+(.+)$",
            r"what\s+(?:licenses?|services|products|subscriptions)\s+does\s+(.+?)\s+have(?:\s+currently|\s+right\s+now|\s+today)?$",
            r"how many\s+(?:licenses?|seats?)\s+(?:for|at)\s+(.+)$",
        )
...
async def handle_chat(request: ChatRequest) -> ChatResponse:
    ...
    if operation == "customer_360":
        ...
        quantity_by_description: dict[str, float] = {}
        total_quantity = 0.0
        ...
        for product in service_products:
            ...
            description = str(description_value).strip() or "Unnamed service"
            quantity_by_description[description] = (
                quantity_by_description.get(description, 0.0)
                + quantity
            )
