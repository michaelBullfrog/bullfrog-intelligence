...
async def handle_chat(request: ChatRequest) -> ChatResponse:
    ...
    if operation == "customer_360":
        ...
        async def _billing_account_family(
            ...
            """
            Return the parent customer plus any Rev.io Billing customers whose
            relationship fields point back to the parent.
            """
            try:
            ...
            for customer in customers:
                customer_id = _possible_customer_id(customer)
                if customer_id is not None:
                    by_id[customer_id] = customer

            family: list[dict[str, Any]] = []
            parent_row = by_id.get(parent_customer_id)
            family.append(
                parent_row
                or {
        ...
        async def customer_rest_ledger(
            customer_id: int,
            *,
            ...
            """
            Build a customer-family ledger from Rev.io Billing.

            Sources:
            - REST Charges
            - REST Credits
            - SOAP Transactions_Query when available, used to add transaction
              types such as PAYMENT / ADJUSTMENT that are not represented by
              Charges or Credits.

            The parent customer and every detected child billing customer are
            aggregated into one Customer 360 ledger.
            """
            ...
            async def fetch_account(
                ...
                soap_entries: list[dict[str, Any]] = []
                soap_error: str | None = None

                charges, credits = await _rest_rows_for_customer(
                    cid,
                    created_date_start=created_date_start,
                    created_date_end=created_date_end,
                    page_size=page_size,
                ...
                if settings.revio_transactions_use_soap:
                    try:
                        ...
                        soap_entries = [
                            revio._normalize_soap_transaction(row)
                            for row in soap_rows
                        ]
                    except Exception as exc:
                        soap_error = str(exc)

                return cid, charges, credits, soap_entries, soap_error
            ...
            # Used to avoid double-counting a Charge/Credit that appears in
            # both REST and Transactions_Query.
            ...
            def add_entry(
                ...
                *,
                ...
                entry["billing_account_name"] = account_name

                signature = (
                    str(entry.get("entry_type") or "").upper(),
            ...
            for (
                ...
                soap_entry_count += len(soap_entries)

                if soap_error:
                    soap_errors[str(cid)] = soap_error

                for row in charges:
                    add_entry(
                        revio._normalize_ledger_entry(row, "CHARGE"),
                        cid=cid,
                    )

                for row in credits:
                    add_entry(
                        revio._normalize_ledger_entry(row, "CREDIT"),
                        cid=cid,
                    )

                # SOAP is additive here. It can provide PAYMENT / ADJUSTMENT
                # rows that do not exist in REST Charges/Credits.
                for entry in soap_entries:
                    add_entry(entry, cid=cid)
            ...
            for entry in entries:
                signed = entry.get("signed_amount")
                if signed in (None, ""):
                    entry_type = str(
                ...
                running_balance += float(signed or 0)
            ...
            total_charges = sum(
                ...
                for entry in entries
                if str(entry.get("entry_type") or "").upper()
                == "CHARGE"
            )
            total_credits = sum(
                abs(float(entry.get("amount") or 0))
            ...
            total_adjustments = sum(
                float(entry.get("signed_amount") or 0)
                for entry in entries
                if str(entry.get("entry_type") or "").upper()
                == "ADJUSTMENT"
            )
            ...
            return {
                ...
                "soap_entry_count": soap_entry_count,
                ...
                "ledger_running_balance": round(running_balance, 2),
                "payment_data_included": total_payments > 0,
                "ledger_source": (
                    "parent_child_rest_plus_soap"
                    if soap_entry_count
                    else "parent_child_rest_charges_credits"
                ),
                ...
                "billing_accounts": [
                    {
                        "customer_id": cid,
                        "customer_name": account_names.get(cid),
                    }
        ...
        if (
            ...
            try:
                all_time_billing = await customer_rest_ledger(
                    billing_customer_id,
                    page_size=500,
                    max_pages=40,
                )
