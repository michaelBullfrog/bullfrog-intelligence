...
def _fallback_summary(
    ...
    if operation == "revio_billing_search_customers":
        ...
        if count == 0:
            return (
                f"I couldn't find a Rev.io Billing customer matching {query!r}."
                if query
                else "No Rev.io Billing customers matched that search."
            )
        ...
        if count == 1:
            ...
            if not name or str(name).strip().casefold() == "unnamed customer":
                try:
                    name = revio._billing_customer_name(customer)
                except Exception:
                    name = query or "the matching customer"

            return (
                f"I found 1 Rev.io Billing customer matching {query!r}: {name}."
                if query
                else f"I found 1 Rev.io Billing customer: {name}."
            )

        return (
            f"I found {count:,} Rev.io Billing customers matching {query!r}."
            if query
            else f"I found {count:,} Rev.io Billing customers."
        )
...
async def summarize(
    ...
    # Customer searches are straightforward live-data lookups; do not spend
    # another OpenAI request just to narrate the returned records.
    if operation == "revio_billing_search_customers":
        return _fallback_summary(
            operation,
            data,
            RuntimeError("deterministic customer summary"),
        )
