...
def _fallback_plan_from_message(message: str) -> dict[str, Any]:
    ...
    customer_patterns = (
        r"(?:search|find|look up|lookup|show|get)\s+(?:for\s+)?(?:customer|company|account)\s+(.+)$",
        r"(?:search|find|look up|lookup|show|get)\s+(.+?)\s+(?:customer|company|account)$",
    ...
    # Short bare company names from the customer-search UI should also route
    # directly to Rev.io Billing, e.g. "The Commonwell".
...
async def handle_chat(request: ChatRequest) -> ChatResponse:
    ...
    if operation == "customer_360":
        ...
        billing_customers: list[dict[str, Any]] = []
        psa_customers: list[dict[str, Any]] = []

        async def safe_billing_search() -> list[dict[str, Any]]:
            try:
                return await revio.search_billing_customers(
        ...
        async def safe_psa_search() -> list[dict[str, Any]]:
            if not search_query:
                return []
            try:
                return await revio.search_customers(
                    query=search_query,
                    page=1,
                    per_page=100,
                )
        ...
        billing_customers, psa_customers = await asyncio.gather(
            safe_billing_search(),
            safe_psa_search(),
        )
        ...
        # Prefer PSA records because they generally expose the actual company
        # name more reliably than Billing search rows.
        for customer in psa_customers:
            if not isinstance(customer, dict):
                continue

            display_name = revio._customer_name(customer)
            if (
                search_query
            ...
            normalized["customer_name"] = display_name
            normalized["customerName"] = display_name
            normalized["CustomerName"] = display_name
            normalized["name"] = display_name
            normalized["Name"] = display_name
            normalized["companyName"] = display_name
            normalized["CompanyName"] = display_name
            normalized["source"] = "Rev.io PSA"
        ...
        # Add Billing matches only when they have a meaningful customer name.
        for customer in billing_customers:
            if not isinstance(customer, dict):
                continue

            display_name = revio._billing_customer_name(customer).strip()
            if (
                not display_name
                or display_name.casefold() == "unnamed customer"
            ):
                continue
            if (
                search_query
            ...
            normalized["companyName"] = display_name
            normalized["CompanyName"] = display_name
            normalized["accountName"] = display_name
            normalized["AccountName"] = display_name
            normalized["source"] = "Rev.io Billing"
            normalized_customers.append(normalized)

        customers = normalized_customers

        # A partial customer-name search should behave like entity resolution:
        # return every plausible Rev.io customer whose company name contains
        # the user's inquiry and let the user choose the intended account.
            ...
            "customer_matches": [
                {
                    ...
                    "source": customer.get("source") or "Rev.io",
                }
                for customer in customers
            ],
            "count": len(customers),
            "source": "Rev.io customer directories",
            "presentation_mode": (
        ...
        if not customers:
            answer = (
                f"I couldn't find a customer with {search_query!r} "
            ...
            customer_name = (
                customers[0].get("customer_name")
                or customers[0].get("customerName")
                or customers[0].get("CustomerName")
                or customers[0].get("name")
                or customers[0].get("Name")
                or search_query
                or "the matching customer"
            )
            answer = (
        ...
        # Customer search is complete here. Return immediately so this request
        # can never be overwritten by a later service-product/license branch.
        _save_conversation(
            conversation_id,
            question=request.message,
            answer=answer,
