...
Use revio_billing_search_contacts for billing/customer contacts. When the user asks for contacts at a company, put the company name in customer_name, not query. Use query only for a person's name, email address, or phone number.
...
a market. All must return the combined US and Canada PostgreSQL renewal dataset. Valid renewal_scope values are all, past_due, next_30, next_60,
...
async def handle_chat(request: ChatRequest) -> ChatResponse:
    ...
    if operation == "company_health_snapshot":
        ...
        normalized_markets = {
            "Canada"
            if str(market).strip().casefold() in {"canada", "ca", "can"}
            else "US"
            if str(market).strip().casefold()
            in {"us", "usa", "united states", "united states of america"}
            else str(market).strip()
        ...
        market_records = [
            ...
            if str(record.get("market") or "").strip()
            in normalized_markets
        ]
        ...
        for record in records:
            market = str(record.get("market") or "Unknown").strip()
        ...
        if customer_id_value is not None:
            ...
            customer_id = None
            company_query = customer_name or contact_query
            if company_query:
                resolution = await revio.resolve_billing_customer(
                    company_query
                )
        ...
        if customer_id_value is not None:
            ...
            company_query = customer_name or contact_query
            if company_query:
                resolution = await revio.resolve_customer(company_query)
                if resolution.get("resolved"):
                    customer_id = int(resolution["customer_id"])
                    resolved_customer_name = str(
                        resolution["customer_name"]
