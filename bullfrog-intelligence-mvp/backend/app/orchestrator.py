...
async def handle_chat(request: ChatRequest) -> ChatResponse:
    ...
    if operation == "customer_360":
        ...
        async def all_billing_customers(
            *,
            page_size: int = 500,
            max_pages: int = 100,
        ) -> list[dict[str, Any]]:
        ...
        async def all_psa_customer_matches() -> list[dict[str, Any]]:
            if not search_query:
                return []
        ...
        billing_directory, psa_customers = await asyncio.gather(
            all_billing_customers(),
            all_psa_customer_matches(),
        )

        normalized_customers: list[dict[str, Any]] = []
        seen_names: set[str] = set()

        # Billing is the primary source for this search. Match the user's text
        # against the ACTUAL Rev.io Billing customer/company name locally.
        for customer in billing_directory:
            if not isinstance(customer, dict):
                continue
            ...
            if search_query and needle not in display_name.casefold():
                continue
        ...
        # PSA is a fallback/secondary directory so customer searches can still
        # show a useful name when Billing lacks one.
        for customer in psa_customers:
            if not isinstance(customer, dict):
                continue

            display_name = revio._customer_name(customer).strip()
        ...
        data = {
            ...
            "count": len(customers),
            "source": "Rev.io Billing customer directory",
            "presentation_mode": (
        ...
        if not customers:
            ...
            names = [
                str(customer.get("customer_name") or "Unnamed customer")
                for customer in customers
            ]
            answer = (
        ...
        return ChatResponse(
            ...
            sources=[
                SourceReference(
                    ...
                    label="Customer directory search",
                )
            ],
        )
