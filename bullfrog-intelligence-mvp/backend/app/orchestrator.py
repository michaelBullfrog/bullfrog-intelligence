...
def _fallback_summary(
    ...
    if operation == "revio_billing_search_customers":
        customers = data.get("customers") or []
        count = int(data.get("count") or len(customers))
        query = str(data.get("query") or "").strip()

        if count == 0:
            if query:
                return (
                    "I couldn't find a Rev.io Billing customer matching "
                    f"{query!r}."
                )
            return "No Rev.io Billing customers matched that search."
        ...
        for customer in customers[:10]:
            ...
            try:
                name = revio._billing_customer_name(customer)
            except Exception:
                name = "Unnamed customer"

            if (
                name
                and name.casefold() != "unnamed customer"
                and name not in names
            ):
                names.append(name)

        if count == 1:
            name = names[0] if names else (query or "the matching customer")
            if query:
                return (
                    f"I found 1 Rev.io Billing customer matching {query!r}: "
                    f"{name}."
                )
            return f"I found 1 Rev.io Billing customer: {name}."

        if query:
            answer = (
                f"I found {count:,} Rev.io Billing customers matching "
                f"{query!r}."
            )
        else:
            answer = f"I found {count:,} Rev.io Billing customers."
    ...
    source = str(data.get("source") or "live business data")
    return (
        f"{records:,} records returned from {source}. "
...
async def summarize(
    ...
    """
    Structured business-data operations are summarized locally.

    OpenAI is reserved for general/ambiguous chat only, so live Rev.io,
    CCW-R, Customer 360, ticket, project, opportunity, billing, renewal,
    contact, and report searches continue working during OpenAI 429s.
    """
    if operation != "general_chat":
        return _fallback_summary(
            operation,
            data,
            RuntimeError("deterministic structured-data summary"),
        )
    ...
    try:
        ...
        if exc.response.status_code != 429:
            raise
        return (
            "Ribbit's AI chat service is temporarily rate-limited. "
            "Live business-data searches remain available."
        )
...
def _fallback_plan_from_message(message: str) -> dict[str, Any]:
    ...
    # Customer lookup/search should never require OpenAI planning.
    # Accept both terse searches ("Mohawk") and natural wording such as
    # "can you find Mohawk?", "search for Mohawk", or "pull up Mohawk".
    customer_name: str | None = None

    customer_patterns = (
        r"(?:can\s+you\s+|could\s+you\s+|please\s+)?(?:search|find|look\s+up|lookup|pull\s+up|pull|show|get)\s+(?:for\s+)?(?:customer|company|account)?\s*(.+)$",
        r"(?:can\s+you\s+|could\s+you\s+|please\s+)?(?:show|get|find)\s+me\s+(?:the\s+)?(?:customer|company|account)?\s*(.+)$",
        r"(?:customer|company|account)\s+(?:named\s+|called\s+|for\s+)?(.+)$",
    )
    ...
    if customer_name:
        return {
            "operation": "revio_billing_search_customers",
            "arguments": {
                "query": customer_name,
                "page": 1,
                "page_size": 100,
    ...
    # Short messages that are not clearly another supported business-data
    # operation are treated as customer-name inquiries. This is intentional:
    # Ribbit is used heavily as a customer lookup tool, and customer search
    # should remain available even when OpenAI is rate-limited.
    bare_words = route_text.split()

    clearly_general = any(
        route_lowered.startswith(marker)
        for marker in (
            "how do ",
            "how can ",
            "why ",
    ...
    structured_keyword = any(
        keyword in route_lowered
        for keyword in (
            "ticket",
            "project",
            "opportunit",
    ...
    if (
        ...
        and not clearly_general
        and not structured_keyword
    ):
        return {
            "operation": "revio_billing_search_customers",
            "arguments": {
...
async def handle_chat(request: ChatRequest) -> ChatResponse:
    ...
    if _looks_like_pdf_followup(request.message, bool(previous_data)):
        ...
        if deterministic_plan.get("operation") != "general_chat":
            ...
            try:
                ...
                # Last-resort business lookup protection: a short phrase that
                # reached the planner should still be usable as a Rev.io
                # customer-name inquiry instead of surfacing a 429 message.
                if plan.get("operation") == "general_chat":
                    fallback_text = " ".join(
                    ...
                    if (
                        fallback_text
                        and len(fallback_text.split()) <= 10
                        and len(fallback_text) <= 120
                    ):
                        plan = {
                            "operation": "revio_billing_search_customers",
                            "arguments": {
    ...
    if operation == "customer_360":
        ...
        resolution = await revio.resolve_customer_query(customer_name)
        matches = resolution.get("matches") or []

        if not resolution.get("resolved"):
            names = [
                str(match.get("customer_name") or "").strip()
                for match in matches[:10]
                if isinstance(match, dict)
                and str(match.get("customer_name") or "").strip()
            ]

            data = {
                "customer_query": customer_name,
                "customer_matches": matches[:10],
            ...
            return ChatResponse(
                ...
                sources=[
                    SourceReference(
                        system="Rev.io",
                        label="Centralized customer entity resolution",
                    )
                ],
            )

        canonical_name = str(
            resolution.get("customer_name") or customer_name
        ).strip()
        psa_customer_id = (
            int(resolution["psa_customer_id"])
            if resolution.get("psa_customer_id") is not None
            else None
        )
        billing_customer_id = (
            int(resolution["billing_customer_id"])
            if resolution.get("billing_customer_id") is not None
            else None
        )
        ...
        search_query = str(args.get("query") or "").strip()

        resolution = await revio.resolve_customer_query(search_query)
        matches = resolution.get("matches") or []
        ...
        for match in matches:
            ...
            customer_name = str(
                match.get("customer_name") or ""
            ).strip()
            customer_id = (
                match.get("billing_customer_id")
                or match.get("psa_customer_id")
            )
            ...
            customers.append(
                {
                    ...
                    "name": customer_name,
                    "Name": customer_name,
                    "billing_customer_id": match.get(
                        "billing_customer_id"
                    ),
                    "psa_customer_id": match.get(
                        "psa_customer_id"
                    ),
                    "sources": match.get("sources") or [],
                    "source": ", ".join(
                        match.get("sources") or ["Rev.io"]
                    ),
                }
            )

        multiple_matches = len(customers) > 1
        data = {
            "query": search_query,
            "customer_query": search_query,
            "customers": customers,
            "customer_matches": customers,
            "count": len(customers),
            "source": "Rev.io centralized customer resolver",
            "presentation_mode": (
                "customer_confirmation"
                if multiple_matches
                else "customer_search"
            ),
            "selection_required": multiple_matches,
            "entity_resolution": {
        ...
        if not customers:
            answer = (
                f"I couldn't find a Rev.io customer with {search_query!r} "
                "in the customer name."
                if search_query
                else "No customers matched that search."
            )
        elif len(customers) == 1:
            answer = (
                f"I found 1 Rev.io customer matching {search_query!r}: "
                f"{customers[0]['customer_name']}."
            )
        else:
            answer = (
                f"I found {len(customers)} Rev.io customers with "
                f"{search_query!r} in the customer name. "
                "Choose the customer you want:\n\n- "
                + "\n- ".join(
        ...
        # Never attach stale structured results to a failed/general request.
        # Otherwise an old customer card can appear beside a 429 message.
        data = {}
        _save_conversation(
            conversation_id,
            question=request.message,
