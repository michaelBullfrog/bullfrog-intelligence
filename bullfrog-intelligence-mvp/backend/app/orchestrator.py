    ...
    if isinstance(company, dict):
        ...
        reports.append(
            _standard_report(
                ...
                source="Rev.io Billing Ledger",
                period=(
                    f"Last {int(billing.get('period_days') or 30)} days"
                ),
                summary=(
                    f"${net:,.2f} in net billing activity across "
                    f"{int(billing.get('customers_with_activity') or 0)} "
                    "customers."
                ),
                kpis=[
                    {
        ...
        due_30 = int(renewals.get("due_0_30") or 0)
        due_90 = int(renewals.get("due_next_90") or 0)
        overdue = int(renewals.get("actionable_overdue") or 0)
        renewal_attention: list[dict[str, Any]] = []
        if overdue:
            renewal_attention.append(
                {
                    "severity": "Critical",
                    "title": f"{overdue} renewals are overdue",
                    "detail": "Confirm ownership and disposition.",
                }
            )
        if due_30:
            renewal_attention.append(
                {
                    "severity": "High",
                    "title": f"{due_30} renewals are due in 30 days",
                    "detail": "Prioritize outreach and quote preparation.",
                }
            )

        reports.append(
            _standard_report(
                report_type="renewal_summary",
                title="Cisco Renewal Summary",
                source=renewals.get("source") or "Ribbit PostgreSQL",
                period="Next 90 days",
                last_refreshed=renewals.get("last_refreshed"),
                summary=f"{due_90} subscriptions are due within 90 days.",
                attention_items=renewal_attention,
                kpis=[
                    {
                    ...
                    {
                        "label": "US",
                        "value": renewals.get("us_subscriptions") or 0,
                        "format": "number",
                    },
                    {
                        "label": "Canada",
                        "value": renewals.get("canada_subscriptions") or 0,
                        "format": "number",
                    },
                ],
            )
