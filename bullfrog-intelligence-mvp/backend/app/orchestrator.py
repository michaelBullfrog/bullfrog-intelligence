...
async def handle_chat(request: ChatRequest) -> ChatResponse:
    ...
    if operation == "company_health_snapshot":
        ...
        results = await asyncio.gather(
            ...
            safe_call(
                "Cisco CCW-R",
                ccwr.search_full_normalized(
                    markets=markets,
                    lookback_days=ccwr_lookback_days,
                    window_days=settings.cisco_window_days,
                    page_size=settings.cisco_max_page_size,
                    max_pages_per_window=(
                        settings.cisco_max_pages_per_window
                    ),
                    max_records=(
                        settings.cisco_max_interactive_records
                    ),
                ),
                {
                    "ccwr_renewals": [],
        ...
        ccwr_result = values["Cisco CCW-R"]

        data = _build_company_snapshot(
            period_days=period_days,
            tickets=values["Rev.io PSA tickets"],
            projects=values["Rev.io projects"],
            opportunities=values[
                "Rev.io opportunities"
            ],
            billing_customers=values[
                "Rev.io Billing customers"
            ],
            ...
            renewals=(
                ccwr_result.get("ccwr_renewals") or []
            ),
            renewal_summary=(
