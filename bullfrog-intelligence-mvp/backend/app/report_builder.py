...
def _dataset_insights(
    ...
    if data_type == "company_health":
        company = data.get("company_health") or {}
        ...
        return [
            (
                "Health score",
                f"{int(company.get('score') or 0)}/100",
                str(company.get("rating") or "Unknown"),
            ),
            (
                "Active tickets",
                f"{int(support.get('active_tickets') or 0):,}",
                "Current support workload",
            ),
            (
                "Needs review",
                f"{int(support.get('needs_review') or 0):,}",
                "Support attention required",
            ),
            (
                "Active projects",
                f"{int(projects.get('active_projects') or 0):,}",
                "Projects in progress",
            ),
            (
                "Opportunities",
                f"{int(sales.get('open_opportunities') or 0):,}",
                "Open sales pipeline",
            ),
            (
                "Pipeline value",
                f"${float(sales.get('pipeline_value') or 0):,.0f}",
                "Estimated open value",
            ),
            (
                "Active services",
                f"{int(billing.get('active_service_products') or 0):,}",
                "Billing service products",
            ),
            (
                "Renewals due 90d",
                f"{int(renewals.get('due_next_90') or 0):,}",
                "Upcoming Cisco renewals",
            ),
        ]
